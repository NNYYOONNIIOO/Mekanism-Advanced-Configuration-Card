...
public class PacketBatchUpgrade implements IMessageHandler<PacketBatchUpgrade.BatchUpgradeMessage, IMessage> {
    ...
    private void doUpgrade(EntityPlayer player, IUpgradeTile upgradeTile, ItemStack bagStack) {
        ...
        for (Upgrade upgrade : Upgrade.getRegisteredUpgrades()) {
            ...
            if (!component.canInstall(upgrade)) continue;
    ...
    private void doUnload(EntityPlayer player, IUpgradeTile upgradeTile, ItemStack bagStack) {
        ...
        for (Upgrade upgrade : Upgrade.getRegisteredUpgrades()) {
            ...
            // Use setUpgrades to directly set count to 0, bypassing output slot
            component.setUpgrades(upgrade, 0);
            changed = true;

            if (player.isCreative()) continue;

            ItemStack upgradeStack = upgrade.getStack(current);
            if (upgradeStack.isEmpty()) continue;

            int remaining = current;

            // Try to insert into bag handler first
