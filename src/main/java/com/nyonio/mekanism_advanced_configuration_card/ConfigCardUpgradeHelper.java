        ...
        for (Upgrade upgrade : Upgrade.values()) {
            ...
            if (current > actualTarget) {
                ...
                if (player.isCreative() || isCoveredByInfiniteUpgrade(upgrade, hasSuperInfinite, hasInfinite)) {
                    ...
                    ItemStack toInsert = ItemCardSlotBag.copyStackWithSize(upgradeStack, toRemove);
                    ItemStack remainder = giveToInventory(player.inventory, toInsert, configCard);
                    if (remainder.isEmpty()) {
                        for (int i = 0; i < toRemove; i++) {
                            component.removeUpgrade(upgrade, false);
                        }
                    } else {
                        int inserted = toRemove - remainder.getCount();
                        if (inserted > 0) {
    ...
    public static void applyClear(EntityPlayer player, IUpgradeTile tile, ItemStack configCard) {
        ...
        for (Upgrade upgrade : Upgrade.values()) {
            ...
            if (player.isCreative() || isCoveredByInfiniteUpgrade(upgrade, hasSuperInfinite, hasInfinite)) {
                ...
                ItemStack toInsert = ItemCardSlotBag.copyStackWithSize(upgradeStack, current);
                ItemStack remainder = giveToInventory(player.inventory, toInsert, configCard);
                if (remainder.isEmpty()) {
                    for (int i = 0; i < current; i++) {
                        component.removeUpgrade(upgrade, false);
                    ...
                    int inserted = current - remainder.getCount();
                    if (inserted > 0) {
                        for (int i = 0; i < inserted; i++) {
                            component.removeUpgrade(upgrade, false);
                        }
