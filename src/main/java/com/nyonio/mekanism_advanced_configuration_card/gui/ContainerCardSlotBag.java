...
public class ContainerCardSlotBag extends Container implements ISortableContainer {
    ...
    private ItemStack handleBagSlotClick(int slotId, int dragType, EntityPlayer player) {
        ...
        if (heldStack.isEmpty()) {
            ...
            ItemStack taken = ItemCardSlotBag.copyStackWithSize(slotStack, takeCount);
            int newSlotCount = slotStack.getCount() - takeCount;
            if (newSlotCount <= 0) {
                handler.setStackInSlot(slotId, ItemStack.EMPTY);
            } else {
                handler.setStackInSlot(slotId, ItemCardSlotBag.copyStackWithSize(slotStack, newSlotCount));
            }
            ...
            if (slotStack.isEmpty()) {
                ...
                handler.setStackInSlot(slotId, ItemCardSlotBag.copyStackWithSize(heldStack, toPlace));
                heldStack.shrink(toPlace);
                if (heldStack.getCount() <= 0) {
                    player.inventory.setItemStack(ItemStack.EMPTY);
                }
                syncSlot(player, slotId);
                return ItemCardSlotBag.copyStackWithSize(heldStack, toPlace);
                ...
                handler.setStackInSlot(slotId, ItemCardSlotBag.copyStackWithSize(slotStack, slotStack.getCount() + toAdd));
                heldStack.shrink(toAdd);
                if (heldStack.getCount() <= 0) {
                    player.inventory.setItemStack(ItemStack.EMPTY);
                }
                syncSlot(player, slotId);
                return ItemCardSlotBag.copyStackWithSize(slotStack, toAdd);
            } else {
    ...
    private ItemStack handleBagShiftClick(int slotId, EntityPlayer player) {
        ...
        if (slotStack.getCount() <= 0) {
            ...
            handler.setStackInSlot(slotId, ItemCardSlotBag.copyStackWithSize(slotStack, slotStack.getCount()));
        }
        syncSlot(player, slotId);
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (index < 27) {
                ...
                if (stack.getCount() <= 0) {
                    ...
                    handler.setStackInSlot(index, ItemCardSlotBag.copyStackWithSize(stack, stack.getCount()));
                }
            } else {
                ...
                for (int i = 0; i < 27 && remaining > 0; i++) {
                    ...
                    if (bagSlot.isEmpty()) {
                        ...
                        handler.setStackInSlot(i, ItemCardSlotBag.copyStackWithSize(stack, toPlace));
                        remaining -= toPlace;
                        ...
                        if (space > 0) {
                            ...
                            handler.setStackInSlot(i, ItemCardSlotBag.copyStackWithSize(bagSlot, bagSlot.getCount() + toAdd));
                            remaining -= toAdd;
                        }
                    }
                }
